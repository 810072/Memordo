...
# Clova 모델 관련 함수 임포트
from hyper_clova_1_5B import load_clova_model, generate_clova_response

from typing import TypedDict, List
from langgraph.graph import StateGraph, END

# --- Clova 모델 로딩 ---
# 워크플로우가 빌드될 때 한 번만 모델을 로드합니다.
print("--- Clova 모델 로딩 시작 ---")
clova_model, clova_tokenizer = load_clova_model()
print("--- Clova 모델 로딩 완료 ---")
...
PROMPT_TEMPLATES = {
    "expand_note": """당신은 사용자의 질문을 '키워드' 중심으로 재구성하여 벡터 검색 성능을 높이는 전문가입니다.
        ...
        --- 원본 메모 ---
        {original_content}""",
    "expand_question": """당신은 사용자의 질문을 벡터 검색에 더 적합하도록 재작성하는 전문가입니다.
    ...
    "validate_documents": """당신은 주어진 문서가 사용자의 질문에 답변하는 데 도움이 될지 평가하는 전문가입니다.
...
def validate_retrieved_documents(state: GraphState) -> dict:
    print("--- (Node 4) Clova를 사용하여 검색된 문서 유효성 검증 ---")
    ...
    prompt = PROMPT_TEMPLATES["validate_documents"].format(question=question, documents=documents_str)

    try:
        response = generate_clova_response(clova_model, clova_tokenizer, prompt)
        print(f"     - Clova 유효성 검증 모델 응답: '{response}'")
...
def expand_short_notes(state: GraphState) -> dict:
    print("--- (Node 0) Clova를 사용하여 짧은 메모 보강 시작 ---")
    notes = state['notes']
    ...
    for note in notes:
        if len(note['content']) < MIN_CHARS_FOR_EXPANSION:
            ...
            prompt = PROMPT_TEMPLATES["expand_note"].format(original_content=note['content'])
            expanded_content = generate_clova_response(clova_model, clova_tokenizer, prompt)
...
def expand_question(state: GraphState) -> dict:
    print("--- (Node 1) Clova를 사용하여 질문 확장 시작 ---")
    original_question = state['question']

    prompt = PROMPT_TEMPLATES["expand_question"].format(question=original_question)
    expanded_question = generate_clova_response(clova_model, clova_tokenizer, prompt)
...
def prepare_retrieval(state: GraphState) -> dict:
    ...
    if notes_to_add:
        ...
        for note in notes_to_add:
            ...
            doc_id = note['fileName']
            all_documents.append(doc)
            all_ids.append(doc_id)

        if all_documents:
            vectorstore.add_documents(documents=all_documents, ids=all_ids)
            print(f"       - 신규 메모 {len(all_documents)}개를 통문서로 ChromaDB에 추가했습니다.")
...
def first_pass_retrieval(state: GraphState) -> dict:
    ...
    vectorstore_for_query = Chroma(persist_directory=db_path, embedding_function=query_embedding_function)
...
def generate_answer(state: GraphState) -> dict:
    # (이 함수는 Gemini 모델을 사용하므로 변경 없음)
    print("--- (Node 6) Gemini를 사용하여 최종 답변 생성 ---")
    question = state['question']
    ...
    for doc in final_docs:
        ...
        context_part = (f"문서명: {source_file}\n--- 전체 내용 ---\n{original_content}\n--- 검색된 관련 부분 ---\n{doc.page_content}")
