...
class _HistoryPageState extends State<HistoryPage> {
  ...
  Future<void> _loadVisitHistory() async {
    ...
    try {
      ...
      if (res.statusCode != 200) {
        if (!mounted) return;
        setState(() => _status = 'Google Drive 파일 목록 가져오기 실패 (상태: ${res.statusCode})');
    ...
    if (_selectedTimestamps.length == 1) {
      ...
      if (selectedUrl != null && selectedUrl.isNotEmpty) {
        ...
        setState(() {
          ...
          _bottomSectionKey.currentState?.updateSummary('URL 요약 중...\n$selectedUrl');
        });
    ...
    return LeftSidebarLayout(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: _status.contains('불러오는 중...') && _visitHistory.isEmpty
                ? Center(child: Column(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                ...itemsOnDate.map((item) {
                                  final title = item['title']?.toString() ?? item['url']?.toString() ?? '제목 없음';
                                  ...
                                  return ListTile(
                                      ...
                                      child: RichText(
                                        ...
                                        text: TextSpan(
                                          ...
                                          style: TextStyle(
                                            ...
                                            decoration: TextDecoration.none, // 밑줄 제거는 onTap으로
                                          ),
                                    ...
                                    contentPadding: EdgeInsets.symmetric(horizontal: 0, vertical: 0), // 내부 패딩 조절
                                  );
                                }).toList(),
          ...
          CollapsibleBottomSection(
            ...
            onSummarizePressed: _isSummarizing ? null : _handleSummarizeAction, // 요약 중일 때 버튼 비활성화
  ...
  String _formatTime(String? isoString) {
    ...
    try {
      ...
      final ampm = dateTime.hour < 12 || dateTime.hour == 24 ? '오전' : '오후'; // 24시(자정)도 오전으로
      final minute = dateTime.minute.toString().padLeft(2, '0');
      return '$ampm $hour12:$minute';
    } catch (_) {
      // ISO 문자열이 아닌 다른 형식의 시간 정보일 수 있음 (예: HH:mm:ss)
      // 간단히 원본 반환하거나, 추가적인 시간 포맷 파싱 로직 필요
      return isoString.length > 5 ? isoString.substring(isoString.length - 8, isoString.length -3) : isoString;
    }
  }
}
